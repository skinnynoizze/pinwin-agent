...
| `example-place-bet.mjs` | Full flow: fetch games → choose selection → place bet (Pinwin + viem). Env: `POLYGON_RPC_URL`, `BETTOR_PRIVATE_KEY`, `BET_AMOUNT`, optional `CONDITION_ID` / `OUTCOME_ID` / `MIN_ODDS`. |
| `example-claim.mjs` | Claim redeemable bets (Pinwin /agent/claim + viem). Run `example-bets-redeemable.mjs` first to get betIds. Env: `POLYGON_RPC_URL`, `BETTOR_PRIVATE_KEY`, `BET_IDS`. |
